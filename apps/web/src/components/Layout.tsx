...
function NavItem({ to, label, icon, end }: { to: string; label: string; icon: React.ReactNode; end?: boolean }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `group flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 mb-0.5 relative ${
          isActive
    ...
    >
      {({ isActive }) => (
        <>
          ...
          <span className={`transition-colors ${isActive ? 'text-primary' : 'text-sidebar-foreground/50 group-hover:text-sidebar-foreground/80'}`}>
            {icon}
          </span>
          {label}
        </>
...
export function Layout() {
  return (
    <div className="min-h-screen bg-background text-foreground flex">
      {/* Sidebar */}
      <aside className="w-56 border-r border-sidebar-border flex flex-col bg-sidebar shrink-0">
        {/* Logo */}
        <div className="h-14 flex items-center px-4 border-b border-sidebar-border gap-2.5">
          ...
          <span className="text-sm font-semibold text-sidebar-foreground tracking-wide">VeraScore</span>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <header className="h-14 border-b border-sidebar-border flex items-center px-6 bg-sidebar text-sidebar-foreground gap-4 shrink-0">
